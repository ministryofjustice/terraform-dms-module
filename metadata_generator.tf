...
data "aws_iam_policy_document" "independent_metadata_generator_lambda_function" {
  ...
  statement {
    ...
    resources = flatten([
      for b in local.independent_rule_buckets : [
        "arn:aws:s3:::${b}",
        "arn:aws:s3:::${b}/*",
      ]
    ])
...
#trivy:ignore:AVD-AWS-0066 X-Ray tracing not currently required. Logs sent to CloudWatch.
module "independent_metadata_generator" {
  for_each = var.independent_full_loads
  # Commit hash for v7.20.1
  source = "git::https://github.com/terraform-aws-modules/terraform-aws-lambda?ref=84dfbfddf9483bc56afa0aff516177c03652f0c7"

  function_name           = "independent-${each.value.full_load_name}-${var.db}-metadata-generator"
  description             = "Lambda to generate metadata for ${var.db} DMS data output"
  handler                 = "main.handler"
  runtime                 = "python3.12"
  memory_size             = 4096
